...
async function fetchTab(tab) {
  ...
  try {
    if (tab === "trending") {
      ...
      const addrs = (j||[]).slice(0,20).map(t=>t.tokenAddress).filter(Boolean).join(",");
      if (addrs) {
      ...
      const q = tab==="top_gainers"?"solana":"sol";
...
function SkeletonRow() {
  return (
    <div style={{display:"grid",gridTemplateColumns:"32px 1fr 100px 70px 70px 80px 90px 60px",padding:"12px 20px",gap:12,borderBottom:"1px solid rgba(255,255,255,0.03)"}}>
      {[30,140,80,60,60,70,70,50].map((w,i) => (
        <div key={i} style={{height:14,width:w,borderRadius:4,background:"rgba(255,255,255,0.05)"}} />
      ))}
    </div>
  );
}
...
function ChangeCell({ value }) {
  const pos = value >= 0;
  return (
    <div style={{fontFamily:"monospace",fontSize:11,color:pos?"#00ff88":"#ff3355",background:pos?"rgba(0,255,136,0.06)":"rgba(255,51,85,0.06)",borderRadius:4,padding:"3px 6px",textAlign:"center",whiteSpace:"nowrap"}}>
      {fmtChange(value)}
    </div>
  );
}
...
function QuickSwapModal({ token, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",backdropFilter:"blur(12px)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"rgba(10,14,20,0.98)",border:"1px solid rgba(0,255,136,0.2)",borderRadius:16,padding:28,width:"100%",maxWidth:400,fontFamily:"monospace",boxShadow:"0 0 60px rgba(0,255,136,0.1)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
          ...
          <button onClick={onClose} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:6,padding:"4px 10px",color:"#666",cursor:"pointer"}}>x</button>
        </div>
        <div style={{marginBottom:16,padding:"12px 14px",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <input defaultValue="0.1" style={{background:"transparent",border:"none",color:"#fff",fontSize:20,fontFamily:"monospace",outline:"none",width:120}} />
          <span style={{background:"rgba(255,255,255,0.08)",borderRadius:6,padding:"4px 10px",color:"#aaa",fontSize:13}}>SOL</span>
        </div>
        ...
        <div style={{marginBottom:20,padding:"12px 14px",background:"rgba(0,255,136,0.03)",border:"1px solid rgba(0,255,136,0.12)",borderRadius:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <span style={{background:"rgba(0,255,136,0.08)",borderRadius:6,padding:"4px 10px",color:"#00ff88",fontSize:13}}>{token.symbol}</span>
        </div>
        <button style={{width:"100%",padding:14,background:"rgba(0,255,136,0.15)",border:"1px solid rgba(0,255,136,0.4)",borderRadius:10,color:"#00ff88",fontSize:13,letterSpacing:2,fontFamily:"monospace",cursor:"pointer",fontWeight:600}}>
...
export default function MarketDashboard({ onTokenSelect }) {
  ...
  useEffect(() => {
    fetchData(activeTab);
    const iv = setInterval(() => fetchData(activeTab), 30000);
  ...
  const cols = isMobile
    ? "32px 1fr 90px 70px 70px 60px"
    : "32px 1fr 100px 70px 70px 80px 90px 90px 60px";

  const headers = isMobile
    ? ["#","TOKEN","PRICE","1H","24H",""]
    : ["#","TOKEN","PRICE","1H","24H","VOLUME","LIQUIDITY","CHART",""];

  return (
    <div style={{background:"rgba(8,12,20,0.95)",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:16,overflow:"hidden",fontFamily:"monospace",boxShadow:"0 24px 80px rgba(0,0,0,0.6)",position:"relative"}}>
      ...
      <div style={{padding:"20px 20px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:12}}>
          ...
          <input
            ...
            style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,padding:"8px 12px",color:"#c8cad8",fontSize:12,fontFamily:"monospace",outline:"none",width:isMobile?160:220}}
          />
        </div>
        <div style={{display:"flex",gap:2,overflowX:"auto"}}>
          ...
          <button onClick={()=>fetchData(activeTab)}
            style={{marginLeft:"auto",background:"transparent",border:"none",color:"#2a3040",fontSize:14,cursor:"pointer",padding:"10px 12px"}}>
            ↻
          </button>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:cols,padding:"8px 20px",gap:12,borderBottom:"1px solid rgba(255,255,255,0.04)",background:"rgba(0,0,0,0.2)"}}>
        {headers.map((h,i) => (
          <div key={i} style={{color:"#1e2535",fontSize:9,letterSpacing:2}}>{h}</div>
        ))}
      </div>

      <div style={{maxHeight:520,overflowY:"auto",scrollbarWidth:"thin",scrollbarColor:"#1a1f2e transparent"}}>
        {error && (
          <div style={{padding:"40px 20px",textAlign:"center",color:"#ff3355",fontSize:12}}>
            Error: {error}
          </div>
        )}
        {loading && Array.from({length:8},(_,i) => <SkeletonRow key={i} />)}
        {!loading && !error && filtered.length===0 && (
          <div style={{padding:"60px 20px",textAlign:"center",color:"#1e2535",fontSize:12}}>No tokens found</div>
        )}
        {!loading && !error && filtered.map((token,i) => (
          <div key={token.address+i}
            ...
            style={{display:"grid",gridTemplateColumns:cols,padding:"11px 20px",gap:12,borderBottom:"1px solid rgba(255,255,255,0.03)",cursor:"pointer",alignItems:"center",transition:"background 0.15s"}}>
            <span style={{color:"#2a3040",fontSize:12,fontFamily:"monospace"}}>{String(token.rank).padStart(2,"0")}</span>
            <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
              <div style={{width:28,height:28,borderRadius:"50%",background:"#0d1520",border:"1px solid rgba(255,255,255,0.06)",flexShrink:0,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center"}}>
                <img src={token.icon} alt="" width={28} height={28} style={{borderRadius:"50%"}}
            ...
            <div style={{fontFamily:"monospace",fontSize:12,color:"#c8cad8",textAlign:"right"}}>{fmtPrice(token.price)}</div>
            <ChangeCell value={token.change1h} />
            <ChangeCell value={token.change24h} />
            <div style={{fontFamily:"monospace",fontSize:12,color:"#8891a8",textAlign:"right"}}>{fmtUsd(token.volume24h)}</div>
            {!isMobile && (
              <div style={{fontFamily:"monospace",fontSize:12,color:"#556070",textAlign:"right"}}>{fmtUsd(token.liquidity)}</div>
            )}
            {!isMobile && (
            ...
            <div style={{display:"flex",justifyContent:"flex-end"}}>
              <button
                ...
                style={{background:"rgba(0,255,136,0.06)",border:"1px solid rgba(0,255,136,0.15)",borderRadius:6,padding:"5px 12px",color:"#00ff88",fontSize:10,letterSpacing:1,cursor:"pointer"}}>
                BUY
              </button>
            </div>
          </div>
      ...
      <div style={{padding:"10px 20px",borderTop:"1px solid rgba(255,255,255,0.03)",display:"flex",justifyContent:"space-between",background:"rgba(0,0,0,0.2)"}}>
        ...
        <span style={{color:"#131820",fontSize:10}}>{filtered.length} PAIRS</span>
      </div>

      {swapToken && (
        <QuickSwapModal token={swapToken} onClose={()=>setSwapToken(null)} />
      )}
    </div>
  );
}
