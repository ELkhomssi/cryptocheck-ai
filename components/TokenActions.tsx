...
export function CopyAddress({ address }: { address: string }) {
  ...
  return (
    <button
      ...
      title="Copy address"
      style={{
        background: copied ? 'rgba(20,241,149,0.15)' : 'rgba(255,255,255,0.04)',
...
export function ShareVerdict({
  ...
  const shareUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/robot?mint=${mint}`
    : `https://cryptocheck.ai/robot?mint=${mint}`;

  const emoji = score >= 70 ? '🟢' : score >= 40 ? '🟡' : '🔴';
  const verdict = score >= 70 ? 'SAFE' : score >= 40 ? 'CAUTION' : 'DANGER';

  const tweet = encodeURIComponent(
    `Just scanned $${symbol} on @CryptoCheckAI\n\n` +
    `${emoji} Safety Score: ${score}/100 — ${verdict}\n` +
    `⚡ Neural Audit: ${shareUrl}\n\n` +
    `#Solana #CryptoCheck #DeFi #TokenAudit`
  );
  ...
  return (
    <a
      ...
      onMouseEnter={e => {
        (e.currentTarget as HTMLElement).style.background = 'rgba(29,161,242,0.15)';
        (e.currentTarget as HTMLElement).style.boxShadow = '0 0 16px rgba(29,161,242,0.2)';
      }}
      onMouseLeave={e => {
        (e.currentTarget as HTMLElement).style.background = 'rgba(29,161,242,0.08)';
...
async function searchTokens(query: string): Promise<SearchResult[]> {
  ...
  try {
    ...
    const pairs = (json?.pairs || []).slice(0, 6);
    ...
    return pairs
      ...
      .map((p: any) => ({
        ...
        price: p.priceUsd ? `$${parseFloat(p.priceUsd).toFixed(6)}` : null,
        chain: p.chainId,
      }));
  } catch {
    return [];
...
export function GlobalSearchBar({ onSelect }: { onSelect?: (mint: string, token: any) => void }) {
  ...
  const handleSelect = (result: SearchResult) => {
    ...
    setOpen(false);
    if (onSelect) {
      onSelect(result.address, result);
    } else {
      // Navigate to robot page with mint param
      router.push(`/robot?mint=${result.address}`);
    }
  };
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      const q = query.trim();
      if (SOLANA_ADDRESS_REGEX.test(q)) {
        handleSelect({ address: q, name: 'Unknown', symbol: '???' });
      ...
      <div style={{
        ...
        <input
          ...
          style={{
            ...
            fontFamily: "'Share Tech Mono', monospace",
            fontSize: 11, width: 220, letterSpacing: 0.5,
          }}
        />
        {loading && (
          <div style={{
            ...
            >
              <div>
                <div style={{ color: '#e8eaf0', fontSize: 13, fontWeight: 600, fontFamily: 'monospace' }}>
                  {r.symbol}
                  <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: 10, marginLeft: 8 }}>{r.name}</span>
                </div>
                <div style={{ color: 'rgba(255,255,255,0.2)', fontSize: 9, marginTop: 2, fontFamily: 'monospace' }}>
...
// ── URL SYNC HOOK ─────────────────────────────────────────────
// Use this in robot/page.tsx to sync mint ↔ URL
